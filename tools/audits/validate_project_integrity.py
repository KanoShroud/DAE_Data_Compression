...
from research_code.shared.baselines import DFTCompressionBaseline, HadamardProjectionBaseline, PCABaseline
...
from research_code.dae_pipeline.evaluate import _all_pair_robust_mode, _localize_from_tdoa_pairs, gcc_standard
...
from research_code.shared.task_baselines import DirectDFTTDOAEstimator
...
def _static_source_gate():
    files = sorted(ROOT.glob("*.py")) + sorted((ROOT / "research_code").rglob("*.py"))
    for path in files:
        ast.parse(path.read_text(encoding="utf-8-sig"), filename=str(path))
    _check(bool(files), f"all {len(files)} project Python files parse as UTF-8")
...
def _feature_budget_gate(sim):
    ...
    from research_code.v5_compressed_tdoa.evaluate_topk_fairness import (
