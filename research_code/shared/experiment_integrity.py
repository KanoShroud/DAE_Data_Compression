...
def assert_protocol_compatible(reference, current, keys=PROTOCOL_KEYS,
    ...
    from runtime_paths import equivalent_code_fingerprints
    ...
    mismatches = {
        ...
        and not (key == "baseline_code_fingerprint"
                 and equivalent_code_fingerprints(ref[key], cur[key]))
    }
    if mismatches:
        preview = "; ".join(
...
def code_fingerprint(root=None, filenames=None):
    root = Path(root or _LAYOUT_ROOT)
    names = filenames or (
        "research_code/dae_pipeline/main.py", "research_code/shared/signal_gen.py", "research_code/dae_pipeline/train.py", "research_code/dae_pipeline/model.py", "research_code/dae_pipeline/evaluate.py",
        "research_code/shared/baselines.py", "research_code/shared/task_baselines.py", "research_code/v5_compressed_tdoa/compressed_tdoa.py",
        "research_code/v5_compressed_tdoa/topk_localization.py", "research_code/shared/experiment_integrity.py",
    )
