...
直接在PyCharm运行`research_code/raptq/run_relative_phase_rate.py`。正式模式先冻结
...
`research_code/raptq/diagnose_stage0_numerical.py`
...
`research_code/raptq/run_stage0.py`
...
`research_code/raptq/diagnose_sparse_integrator.py`

已完成的36-context纠正性重算入口：

`research_code/raptq/recompute_stage0_sparse_amp_phase.py`

当前已批准、待正式运行的最终可行性入口：

`research_code/raptq/run_relative_phase_rate.py`

该入口不修改`core.py`的冻结积分路径，而是在独立模块
`research_code/raptq/sparse_integrator.py`中使用无冗余Dirichlet stick-breaking
