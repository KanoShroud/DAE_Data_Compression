"""插值频谱伪补全的独立 BPSK 扩展参数扫描。

本脚本不修改既有 ``synthetic_full`` 协议或结果。正式运行包含五类信号、
四个单站频点预算、五个 SNR、五个双站重叠率和每条件 300 次 Monte Carlo
试验，并在原有方法之外显式计算固定 8 倍时域补零密集频谱（TZP-DS）。
运行按条件分块保存，可在意外中断后用 ``--resume`` 继续。
"""
...
def _normalise_spectrum(spectrum: np.ndarray) -> np.ndarray:
    scale = max(np.sqrt(np.mean(np.abs(spectrum) ** 2)), np.finfo(float).tiny)
...
def _add_frequency_noise(
    ...
    noise = np.sqrt(noise_power / 2.0) * (
        rng.normal(size=clean_spectrum.size)
        + 1j * rng.normal(size=clean_spectrum.size)
    )
...
def _controlled_masks(
    ...
    if 2 * selected_bins - common_count > n_fft:
        raise ValueError("N、M 和重叠率不能构造两个严格大小为 M 的集合")
    ...
    return (
        np.sort(np.concatenate((common, a_only))),
        np.sort(np.concatenate((common, b_only))),
    )
...
def main() -> None:
    ...
    run_sweep(config, arguments.resume, arguments.workers)
